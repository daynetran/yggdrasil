...
\textbf{Sources of Aborted Transactions} 1) User explicitly issues rollback; 2) Deadlock; 3) System crash; 4) Integrity constraint violation
\ruler
\textbf{Sources of DBMS Crash} 1) Operator Error; 2) Configuration Error; 3) Soft/Hardware failure
\ruler 
\textbf{No Steal / Force}: This simple yet flawed policy doesn't allow buffer-pool frames with uncommited updates to be replaced (pinned pages limit buffer replacement $\rightarrow$ poor performance) and makes sure every update is "forced" onto the DB disk before commit (lots of commits mean lots of unnecessary I/Os $\rightarrow$ poor performance).
\ruler
\textbf{Steal / No Force}:
\ruler
...
1. On any record that is not an END record: add the Xact to the Xact Table (if necessary). Set the lastLSN of the transaction to the LSN of the record you are on.\\
...
\-\hspace{0.5cm} 2. recLSN > LSN. The first update that dirtied the page occurred after this operation. Implies that the operation we are currently at has already made it to disk, other wise it woudl be the recLSN.\\
